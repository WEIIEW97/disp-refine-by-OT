...
class Parameters:
    def __init__(self, max_disparity=64, P1=5, P2=70, csize=(7, 7)):
        """
...
def sgma():
    ...
    parameters = Parameters(max_disparity=disparity, P1=10, P2=120, csize=(7, 7))
    paths = Paths()
...
def sgm(lpath, rpath, blk_size, d_max, p1=10, p2=120, save_images=False):
    t0 = t.time()
    param = Parameters(max_disparity=d_max, P1=p1, P2=p2, csize=(blk_size, blk_size))
    paths = Paths()

    lpp = Path(lpath).parent
    rpp = Path(rpath).parent

    print('\nLoading images...')
    left, right = load_images(lpath, rpath)

    print('\nStarting cost computation...')
    left_cost_volume, right_cost_volume = compute_costs(left, right, param, save_images)
    if save_images:
        left_disparity_map = np.uint8(normalize(np.argmin(left_cost_volume, axis=2), param))
        cv2.imwrite('disp_map_left_cost_volume.png', left_disparity_map)
        right_disparity_map = np.uint8(normalize(np.argmin(right_cost_volume, axis=2), param))
        cv2.imwrite('disp_map_right_cost_volume.png', right_disparity_map)

    print('\nStarting left aggregation computation...')
    left_aggregation_volume = aggregate_costs(left_cost_volume, param, paths)
    print('\nStarting right aggregation computation...')
    right_aggregation_volume = aggregate_costs(right_cost_volume, param, paths)

    print('\nSelecting best disparities...')
    left_disparity_map = np.uint8(normalize(select_disparity(left_aggregation_volume), param))
    right_disparity_map = np.uint8(normalize(select_disparity(right_aggregation_volume), param))
    if save_images:
        cv2.imwrite('left_disp_map_no_post_processing.png', left_disparity_map)
        cv2.imwrite('right_disp_map_no_post_processing.png', right_disparity_map)

    # print('\nApplying median filter...')
    # left_disparity_map = cv2.medianBlur(left_disparity_map, param.bsize[0])
    # right_disparity_map = cv2.medianBlur(right_disparity_map, param.bsize[0])
    cv2.imwrite(f'{lpp}/py_sgm_left.png', left_disparity_map)
    cv2.imwrite(f'{rpp}/py_sgm_right.png', right_disparity_map)
    ...
    t1 = t.time()
    print('\nFin.')
    print('\nTotal execution time = {:.2f}s'.format(t1 - t0))


if __name__ == '__main__':
    lpath = "/home/william/extdisk/data/middlebury/middlebury2014/Bicycle1-perfect/im0.png"
    rpath = "/home/william/extdisk/data/middlebury/middlebury2014/Bicycle1-perfect/im1.png"

    blk_size = 15
    max_d = 384


    sgm(lpath, rpath, blk_size, max_d)